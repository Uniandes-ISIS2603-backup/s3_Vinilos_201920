...
public List<GeneroDetailDTO> generoListEntityListaDTO(List<GeneroEntity> entityList){
 ...
 @GET
    public List<GeneroDTO> getGeneros() {
        ...
        List<GeneroDTO> listaEnvios = listEntityPedidos(logica.getBooks());
        //LOGGER.log(Level.INFO, "PedidoResource getPedidos: output: {0}", listaEnvios);
        return listaEnvios;
    }
    
    private List<GeneroDTO> listEntityPedidos(List<GeneroEntity> entityList) {
        List<GeneroDTO> list = new ArrayList<>();
        for (GeneroEntity entity : entityList) {
            list.add(new GeneroDTO(entity));
        }
        return list;
    }
}
